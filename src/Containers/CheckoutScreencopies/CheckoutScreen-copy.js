...
import { ActivityIndicator, Image, BackHandler, View, TouchableNativeFeedback, Alert, NetInfo } from "react-native";
...
class CheckoutScreen extends React.Component {
    ...
    this.state = {
				...
				persons: 0,
				showToast: false, //уведомления об отсутсвии интернета
				btndisabled: false,
				loading: false
		};
  }
  ...
  //Отправка заказа в базу
	addOrder = (props) => {
		...
		/*disablebtnfunction = () => { //Предотвращение двойного клика(двойной отправки в базу)
		...
		loadingtrue = () => { //Установление ActivityIndicator в положение true после нажатия на кнопку продолжить
			return new Promise((resolve, reject) => {
		...
		function isConnectionOnline() { //Проверка на наличие соединения
			return new Promise((resolve, reject) => {
		...
		function setOrderwithConnectionChecking() { //Отправка заказа в базу

					function addOrderData(indexData) { //Записываем в базу
							return new Promise((resolve, reject) => {
			...
			<Container>
				...
				{!this.state.loading ? (
	              ...
	              <View style={styles.btninputblock}>
				            ...
				            <Input disabled placeholder={this.state.persons==0 ? ('Кол-во персон') : (this.state.persons+'')} style={styles.input0persons}/>
				          </Item>
