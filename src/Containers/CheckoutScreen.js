...
import { Image, BackHandler, View, TouchableNativeFeedback, Alert, ActivityIndicator, NetInfo } from "react-native";
import { Container, Header, Content, Footer, Input, List, ListItem, Item, Text, Left, Right, Body, Icon, Button, Title, Toast, Separator, Thumbnail } from 'native-base';
...
class CheckoutScreen extends React.Component {
	...
	gobackwithAlert() {
			Alert.alert(
				...
				[
					{text: 'Оформить заказ', onPress: () =>
						console.log('Продолжение оформления заказа')
					},
					{text: 'Выйти', onPress: () =>
  ...
  //Отправка заказа в базу
	addOrder = (props) => {
		...
		setOrderwithConnectionChecking = () => {
				...
				disablebtnfunction = () => { //Отключение кнопки продолжить
				...
				checkingisInputsCorrect = () => { //Отключение кнопки продолжить
					return new Promise((resolve, reject) => {
							if(!this.state.namestatussuccess || !this.state.phonestatussuccess || !this.state.adressstatussuccess){
				...
				loadingActivitytrue = () => { //Отключение кнопки продолжить
					return new Promise((resolve, reject) => {
				...
				addOrderData = () => { //Записываем в базу
						return new Promise((resolve, reject) => {
				...
				*/
				sendGridEmail = (/*,cart,shipping,subTotal,callback*/) => {
					return new Promise((resolve, reject) => {
						...
						//Iterate the messages
						 ...
						 message+=("Итог: "+this.props.navigation.state.params.preTotal+"р"+"\n");
						 message+=("С учетом скидки: "+this.props.navigation.state.params.total+"р");
						 ...
						 fetch('https://api.sendgrid.com/v3/mail/send', {
								...
								"personalizations": [
									{
										...
										"subject": "Новый заказ №"+this.state.orderid+' от '+this.state.nameClient+' '+this.state.phoneClient
									}
								],
								"from": {
							...
							resolve();
						}).catch((error)=>{console.log("Не удалось отправить e-mail")})
					});
				}


				loadingActivityfalse = () => { //Отключение кнопки продолжить
					return new Promise((resolve, reject) => {
				...
				clearCartandAlertandNavigate = () => { //Очистка корзины и навигация на главную страницу
					return new Promise((resolve, reject) => {
							Alert.alert(
		...
		if(this.state.deliveryoptions.value=='Доставка'){
				if(!this.state.nameClient || !this.state.phoneClient || !this.state.adressClient){
					...
					if(!this.state.adressClient){
						if(this.state.deliveryoptions.value=='Доставка'){
							this.setState({adressstatuserror: true, adressstatussuccess: false});
		...
		return (
			<Container>
              ...
              <View style={styles.btninputblock}>
			            ...
			            <Input disabled placeholder={'Персон: '+this.state.persons} style={styles.input0persons}/>
			          </Item>
							  <View style={styles.btnblock}>
										<Button style={styles.btnminus} onPress={() => this.decrement()}>
							...
							<Item error={this.state.namestatuserror} success={this.state.namestatussuccess} style={styles.iteminput}>
		            ...
		            <Input
							...
							<Item ref={elem => this.iteminput = elem} error={this.state.phonestatuserror} success={this.state.phonestatussuccess} style={styles.iteminput}>
		            ...
		            <Input
								...
								/>
								{this.state.phonestatussuccess && <Icon name='checkmark-circle' />}
								{this.state.phonestatuserror && <Icon name='close-circle' />}
		          </Item>
							{this.state.deliveryoptions.value=='Доставка' &&
							<Item error={this.state.adressstatuserror} success={this.state.adressstatussuccess} style={styles.iteminput}>
		            ...
		            <Input
									...
									maxLength={100}
								/>
								{this.state.adressstatussuccess && <Icon name='checkmark-circle' />}
				...
				{this.state.loading==false &&
					<Footer style={styles.buttoncontinueblock}>
							<Button disabled={this.state.btndisabled} full style={styles.buttoncontinue} onPress={() => this.addOrder(this.props)}>
