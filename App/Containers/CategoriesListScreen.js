...
import { Image, BackHandler, FlatList, TouchableOpacity, ActivityIndicator } from "react-native";
import { Text, View, Container,	Header,	Content, Title,	Left,	Right, Body, Badge, Icon, List, ListItem, Button, Thumbnail} from "native-base";
...
class CategoriesListScreen extends React.Component {
	...
	handlePressCategory = (item) => {
	  this.props.navigation.navigate("CategoryScreen", {name: item.name, ref: item.itemref});
	};
		...
		return (
			<Container>
				...
				<FlatList
					...
					renderItem={({ item }) => (
						<ListItem style={styles.row0} onPress={()=>{this.handlePressCategory(item)}}>
							<Thumbnail circle size={51} source={require('../Images/rolls.png')} />
							<Text onPress={()=>{this.handlePressCategory(item)}} style={styles.row0text} uppercase={true}>{item.name}</Text>
						</ListItem>
					)}
