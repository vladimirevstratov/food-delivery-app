		...
		return (
			<Container>
				...
				<Content style={styles.content}>
						<View style={styles.emptycart}>
							...
							<View style={styles.blockbuttontomenu}>
								<Button style={styles.buttontomenu} onPress={() => this.navigatetoMenu()}><Text style={styles.buttontextemptycart}>В МЕНЮ</Text></Button>
							</View>
						</View>
				</Content>
				) : (
				...
				<FlatList
					...
					renderItem={({ item, index }) => (
								<SwipeRowCart onRef={ref => (this.child = ref)} damping={this.state.damping} key={item.id} tension={this.state.tension} action={this.setQty} index={index} qty={item.qty}>
					            ...
					            <View>
					              <Text style={styles.rowTitle}>{item.name}</Text>
										...
										<View style={styles.rowButton}>
											<View style={styles.button0}/* onPress={() => this.onPressSnapTo()}*/>
												<Text style={styles.buttontext0}>{item.qty} x {item.price} р</Text>
											</View>
										</View>
				          </View>
				        </SwipeRowCart>
			...
			{this.props.items.length!=0 &&
				<Footer style={styles.footer}>
					<CartTotal skidka={this.props.skidka} tax={tax} total={TOTAL} subTotal={subTotal} />
					<Button full style={styles.buttoncheckout} onPress={() => this.props.navigation.navigate("CheckoutScreen", {preTotal: subTotal, total: TOTAL, skidka: tax})}>
